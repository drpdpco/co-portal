...
var API_MAP = {
  ...
  adminSetTabItemHidden: adminSetTabItemHidden,
...
function ensureSetup_() {
  ...
  // Seed default scrolling ticker text once
  var settingsSh = ss.getSheetByName('Settings');
  var settingsRows = settingsSh.getDataRange().getValues();
  var hasTicker = false;
  for (var s = 1; s < settingsRows.length; s++) if (settingsRows[s][0] === 'ticker') hasTicker = true;
  if (!hasTicker) {
    var defaultTicker = [
      'Vision- To educate students, to be computer technology expert & technocrat, with brainstorming & learning hardware, software, co-curricular and extracurricular concepts.',
      'Mission- To enhance and pacify knowledge thirst in order to strengthen IT and Computer skills in building blocks of nation.'
    ];
    settingsSh.appendRow(['ticker', JSON.stringify(defaultTicker)]);
  }
  ...
  // Migrate old TabItems rows (single "Link" text -> "LinksJson" array; add SortOrder + Hidden columns)
  var tish = ss.getSheetByName('TabItems');
  var tiHeader = tish.getRange(1, 1, 1, Math.max(6, tish.getLastColumn())).getValues()[0];
  if (tiHeader[4] !== 'LinksJson') {
    var tiData = tish.getDataRange().getValues();
    for (var ti = 1; ti < tiData.length; ti++) {
      if (!tiData[ti][0]) continue;
      var oldLink = String(tiData[ti][4] || '').trim();
      var linksArr = oldLink ? [{ title: 'Attachment', url: oldLink }] : [];
      tish.getRange(ti + 1, 5).setValue(JSON.stringify(linksArr));
    }
    tish.getRange(1, 5).setValue('LinksJson');
  }
  if (tish.getLastColumn() < 7 || tish.getRange(1, 7).getValue() !== 'SortOrder') {
    tish.getRange(1, 7).setValue('SortOrder');
    var tiData2 = tish.getDataRange().getValues();
    for (var tj = 1; tj < tiData2.length; tj++) {
      if (!tiData2[tj][0]) continue;
      if (!tiData2[tj][6]) tish.getRange(tj + 1, 7).setValue(tj); // backfill using existing row order
    }
  }
...
function getPortalData(token) {
  try {
    ...
    for (var t = 1; t < ti.length; t++) {
      if (!ti[t][0]) continue;
      var isHidden = !!ti[t][7];
      if (isHidden && s.role !== 'admin') continue;
      var linksArr = [];
      try { linksArr = JSON.parse(ti[t][4] || '[]'); } catch (e3) { linksArr = []; }
      tabItems.push({ id: ti[t][0], tabId: ti[t][1], title: ti[t][2],
                      details: ti[t][3], links: linksArr, posted: fmtDate_(ti[t][5]),
                      order: Number(ti[t][6]) || 0, hidden: isHidden });
    }
...
function getPublicResources() {
  try {
    ...
    // NOTE: Menu Tabs (e.g. "Operating System" -> Curriculum/Syllabus) are
    // intentionally NOT included here — that content requires login. Only the
    // classic Home-Page Top Menu Buttons (TopMenus/Resources) are public.
    return jsonSafe_({ ok: true, menus: menus, rows: out });
  } catch (e) { return { ok: false, msg: e.message, menus: [], rows: [] }; }
}

/* ------------------------------------------------------------------ */
/*  SCROLLING TICKER TEXT (public read, admin write)                   */
/* ------------------------------------------------------------------ */
function getTickerText() {
  try {
    ensureSetup_();
    var d = sheet_('Settings').getDataRange().getValues();
    for (var i = 1; i < d.length; i++) {
      if (d[i][0] === 'ticker') {
        var lines = [];
        try { lines = JSON.parse(d[i][1]); } catch (e2) { lines = []; }
...
function adminSetTicker(token, lines) {
  try {
    requireAdmin_(token);
    if (!lines || !lines.length) return { ok: false, msg: 'Please enter at least one line.' };
    var clean = lines.map(function (s) { return String(s || '').trim(); }).filter(function (s) { return s; });
    if (!clean.length) return { ok: false, msg: 'Please enter at least one line.' };
    var sh = sheet_('Settings');
    var d = sh.getDataRange().getValues();
    var rowIdx = -1;
    for (var i = 1; i < d.length; i++) if (d[i][0] === 'ticker') { rowIdx = i + 1; break; }
    if (rowIdx > 0) sh.getRange(rowIdx, 2).setValue(JSON.stringify(clean));
    else sh.appendRow(['ticker', JSON.stringify(clean)]);
    return { ok: true, msg: 'Ticker text updated.' };
...
/* ------------------------------------------------------------------ */
/* ------------------------------------------------------------------ */
/*  ADMIN — currently logged-in users (active sessions)                */
/* ------------------------------------------------------------------ */
function adminGetActiveSessions(token) {
  try {
    requireAdmin_(token);
    var sh = sheet_('Sessions'), d = sh.getDataRange().getValues(), now = new Date();
    var active = [];
    for (var i = 1; i < d.length; i++) {
      if (!d[i][0]) continue;
      var created = new Date(d[i][4]);
      var hoursElapsed = (now - created) / 3600000;
      if (hoursElapsed <= SESSION_HOURS) {
        active.push({ email: d[i][1], name: d[i][2], role: d[i][3],
                      loginTime: fmtSmart_(created), ts: created.getTime() });
      }
    }
    active.sort(function (a, b) { return a.ts - b.ts; });
    function byRole(role) { return active.filter(function (x) { return x.role === role; }); }
    function rangeOf(list) {
      if (!list.length) return null;
      return { earliest: list[0].loginTime, latest: list[list.length - 1].loginTime };
    }
    var students = byRole('student'), teachers = byRole('teacher'), admins = byRole('admin');
    return jsonSafe_({
      ok: true, sessionHours: SESSION_HOURS, sessions: active,
      studentCount: students.length, studentRange: rangeOf(students),
      teacherCount: teachers.length, teacherRange: rangeOf(teachers),
      adminCount: admins.length, adminRange: rangeOf(admins)
    });
  } catch (e) { return { ok: false, msg: e.message }; }
}
...
function cleanLinks_(links) {
  if (!links || !links.length) return [];
  var out = [];
  for (var i = 0; i < links.length; i++) {
    var url = String((links[i] && links[i].url) || '').trim();
    if (!url) continue;
    var title = String((links[i] && links[i].title) || '').trim() || 'Attachment';
...
function adminAddTabItem(token, f) {
  try {
    ...
    var sh = sheet_('TabItems');
    var d = sh.getDataRange().getValues();
    var maxOrder = 0;
    for (var j = 1; j < d.length; j++) if (Number(d[j][6]) > maxOrder) maxOrder = Number(d[j][6]);
    sh.appendRow(['ITM-' + Date.now(), f.tabId, f.title, f.details || '',
      JSON.stringify(cleanLinks_(f.links)), new Date(), maxOrder + 1, '']);
    return { ok: true, msg: 'Content published on the menu tab.' };
  } catch (e) { return { ok: false, msg: e.message }; }
}

function adminUpdateTabItem(token, id, f) {
  try {
    requireAdmin_(token);
    if (!f || !f.tabId || !f.title) return { ok: false, msg: 'Menu tab and title are required.' };
    ...
    for (var i = 1; i < d.length; i++) {
      if (d[i][0] === id) {
        sh.getRange(i + 1, 2, 1, 4).setValues([[f.tabId, f.title, f.details || '', JSON.stringify(cleanLinks_(f.links))]]);
        return { ok: true, msg: 'Content updated.' };
      }
    }
...
function adminMoveTabItem(token, id, direction) {
  try {
    requireAdmin_(token);
    var sh = sheet_('TabItems'), d = sh.getDataRange().getValues();
    var cur = null, curRow = -1;
    for (var i = 1; i < d.length; i++) if (d[i][0] === id) { cur = d[i]; curRow = i; break; }
    if (!cur) return { ok: false, msg: 'Content not found.' };
    var siblings = [];
    for (var j = 1; j < d.length; j++) if (d[j][1] === cur[1]) siblings.push({ row: j, order: Number(d[j][6]) || 0, id: d[j][0] });
    siblings.sort(function (a, b) { return a.order - b.order; });
    var idx = siblings.findIndex(function (s) { return s.id === id; });
    var swapIdx = direction === 'up' ? idx - 1 : idx + 1;
    if (swapIdx < 0 || swapIdx >= siblings.length) return { ok: true, msg: 'Already at the ' + (direction === 'up' ? 'top' : 'bottom') + '.' };
    var a = siblings[idx], b = siblings[swapIdx];
    sh.getRange(a.row + 1, 7).setValue(b.order);
    sh.getRange(b.row + 1, 7).setValue(a.order);
    return { ok: true, msg: 'Order updated.' };
  } catch (e) { return { ok: false, msg: e.message }; }
}

function adminSetTabItemHidden(token, id, hidden) {
  try {
    requireAdmin_(token);
    var sh = sheet_('TabItems'), d = sh.getDataRange().getValues();
    for (var i = 1; i < d.length; i++) {
      if (d[i][0] === id) {
        sh.getRange(i + 1, 8).setValue(hidden ? '1' : '');
        return { ok: true, msg: hidden ? 'Content hidden from students/teachers.' : 'Content is now visible again.' };
      }
    }
    return { ok: false, msg: 'Content not found.' };
  } catch (e) { return { ok: false, msg: e.message }; }
